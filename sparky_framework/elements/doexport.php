...
function namechange($file,$newname){
	...
	if($content) {
		$content = str_replace('sparky_framework', $alias, $content);
		$content = str_replace('SPARKY_FRAMEWORK', $aliasU, $content);
		$content = str_replace('sparky_framework', $nameL, $content);
		$content = str_replace('SPARKY_FRAMEWORK', $nameU, $content);
		$content = str_replace('sparky_framework', $alias, $content);
		$content = str_replace('SPARKY_FRAMEWORK', $aliasU, $content);
		$content = str_replace('sparky_framework', $newname, $content);
		$content = str_replace('sparky_framework', $newname, $content);
		$content = str_replace('sparky_framework', $newname, $content);
...
foreach($settingXML->config->fields->fieldset as $fieldset){
	foreach($fieldset->field as $field){
		...
		if (isset($_REQUEST['jform']['params'][$field["name"].''])) {
...
try {
    ...
    // delete exported template after packing
